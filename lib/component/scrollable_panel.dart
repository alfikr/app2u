...
class _ScrollablePanelState extends State<ScrollablePanel>
  ...
  void _onDragEnd() {
    ...
    if (fromMaxValue.abs() < _snapThreshold ||
        _animationController.value > half) {
      _animateTo(maxPanelSize);
      return;
    }

    double fromDefaultValue = defaultPanelSize - _animationController.value;
    if (fromDefaultValue.abs() < _snapThreshold ||
        _animationController.value < half) {
...
class _PanelScrollPosition extends ScrollPositionWithSingleContext {
  ...
  void applyUserOffset(double delta) {
    if (!listShouldScroll &&
        (!(controller.value == controller.maxPanelSize ||
                controller.value == controller.minPanelSize) ||
